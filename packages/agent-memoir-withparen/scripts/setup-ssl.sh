...
# SSL Certificate Setup for appchat.withseren.com with PM2 Integration
# This script sets up SSL certificates using Certbot (Let's Encrypt)

set -e

DOMAIN="appchat.withseren.com"
EMAIL="team@withseren.com"  # Replace with your email
NGINX_CONFIG="/home/specialpedrito/agents/packages/agentseren/scripts/nginx-agent.conf"
PM2_SERVICE="app"
...
sudo sed -i "s|/etc/ssl/certs/appchat.withseren.com.pem|/etc/letsencrypt/live/$DOMAIN/fullchain.pem|g" $NGINX_CONFIG
sudo sed -i "s|/etc/ssl/private/appchat.withseren.com.key|/etc/letsencrypt/live/$DOMAIN/privkey.pem|g" $NGINX_CONFIG

# Copy nginx configuration to sites-available
echo "📋 Installing nginx configuration..."
sudo cp $NGINX_CONFIG /etc/nginx/sites-available/appchat.withseren.com
sudo ln -sf /etc/nginx/sites-available/appchat.withseren.com /etc/nginx/sites-enabled/
...
echo "📜 View nginx logs: sudo tail -f /var/log/nginx/appchat.withseren.com.access.log"
