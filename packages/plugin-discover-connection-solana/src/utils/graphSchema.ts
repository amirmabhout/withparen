 ...
 * Note: agentId property used for efficient agent-based filtering in queries
 */
export interface PersonNode {
  type: 'Person';
  entityid: UUID; // ElizaOS entityId of the user
  agentId?: UUID; // Agent that manages this person (used for filtering queries efficiently)

  // Schema.org standard properties
  name?: string;

  // Geospatial coordinates for location-based matching (optional)
...
export interface PlaceNode {
  ...
  // Geospatial coordinates for proximity search indexing
  lat: number; // Latitude
...
interface BaseDimensionNode {
  agentId?: UUID; // Agent that extracted this dimension (for data cleanup)
...
export interface AccountNode {
  type: 'Account';
  agentId?: UUID; // Agent that created this account node (for data cleanup)
...
export interface VerificationDataNode {
  ...
  agentId?: UUID; // Agent that created this verification data (for data cleanup)
...
export interface MatchedWithRelationship {
  ...
  // HumanConnection on-chain data (populated when status transitions to accepted)
  connectionId?: string; // On-chain connection ID from Solana
  pinA?: string; // User A's PIN (given to User B to submit for unlocking rewards)
  pinB?: string; // User B's PIN (given to User A to submit for unlocking rewards)
  pdaWalletA?: string; // User A's PDA wallet address (from)
  pdaWalletB?: string; // User B's PDA wallet address (to)
