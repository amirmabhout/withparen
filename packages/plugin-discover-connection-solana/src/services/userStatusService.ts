...
export enum UserStatus {
  ...
  ACTIVE = 'active', // Completed onboarding, available for matching
  MATCHED = 'matched', // Currently in an active match, not available for new matches
}
...
#   * Get the current status of a user from Memgraph
  ...
  private isValidStatusTransition(currentStatus: UserStatus, newStatus: UserStatus): boolean {
    ...
    const allowedTransitions: Record<UserStatus, UserStatus[]> = {
      ...
      [UserStatus.ACTIVE]: [UserStatus.MATCHED], // Active users can enter matches
      [UserStatus.MATCHED]: [UserStatus.ACTIVE], // Matched users return to active when match ends
    };
  ...
  private isStatusSufficientForAction(
    ...
    const statusHierarchy = {
      ...
      [UserStatus.ACTIVE]: 2,
      [UserStatus.MATCHED]: 2, // Same level as ACTIVE - matched users retain full access
    };
