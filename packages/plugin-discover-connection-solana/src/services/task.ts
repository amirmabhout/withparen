...
import { createMatchExpiryTask } from '../tasks/matchExpiry.js';
import { createMeetingReminderTask } from '../tasks/meetingReminder.js';
...
export class TaskService extends Service {
  ...
  static async start(runtime: IAgentRuntime): Promise<Service> {
    ...
    // Initialize match expiry task
    try {
      await createMatchExpiryTask(runtime);
      logger.info('[discover-connection] TaskService: Match expiry task initialized');
    } catch (error) {
      logger.error(
        `[discover-connection] TaskService: Failed to initialize match expiry task: ${error}`
      );
    }

    // Initialize meeting reminder task
    try {
      await createMeetingReminderTask(runtime);
      logger.info('[discover-connection] TaskService: Meeting reminder task initialized');
    } catch (error) {
      logger.error(
        `[discover-connection] TaskService: Failed to initialize meeting reminder task: ${error}`
      );
    }
