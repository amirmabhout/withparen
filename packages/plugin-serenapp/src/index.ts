...
export async function processAttachments(
  ...
  logger.debug(`[serenapp] Processing ${attachments.length} attachment(s)`);
  ...
  for (const attachment of attachments) {
    try {
      ...
      if (attachment.contentType === ContentType.IMAGE && !attachment.description) {
        logger.debug(`[serenapp] Generating description for image: ${attachment.url}`);
        ...
        try {
          ...
          if (typeof response === 'string') {
            ...
            if (parsedXml?.description && parsedXml?.text) {
              ...
              logger.debug(
                `[serenapp] Generated description: ${processedAttachment.description?.substring(0, 100)}...`
              );
            } else {
              logger.warn(`[serenapp] Failed to parse XML response for image description`);
            }
        ...
        if (isPlainText) {
          logger.debug(`[serenapp] Processing plain text document: ${attachment.url}`);
          ...
          logger.debug(
            `[serenapp] Extracted text content (first 100 chars): ${processedAttachment.text?.substring(0, 100)}...`
          );
        } else {
          logger.warn(`[serenapp] Skipping non-plain-text document: ${contentType}`);
        }
      }
  ...
  logger.debug(`[serenapp] Processed attachments: ${JSON.stringify(processedAttachments)}`);
...
const messageReceivedHandler = async ({
  ...
  try {
    logger.info(`[serenapp] Message received from ${message.entityId} in room ${message.roomId}`);
    ...
    const processingPromise = (async () => {
      try {
        if (message.entityId === runtime.agentId) {
          logger.debug(`[serenapp] Skipping message from self (${runtime.agentId})`);
          throw new Error('Message is from the agent itself');
        }

        logger.debug(
          `[serenapp] Processing message: ${truncateToCompleteSentence(message.content.text || '', 50)}...`
        );

        // First, save the incoming message
        logger.debug('[serenapp] Saving message to memory and embeddings');
        await Promise.all([
        ...
        if (
          ...
          logger.debug(`[serenapp] Ignoring muted room ${message.roomId}`);
          return;
        }
        ...
        logger.debug(`[serenapp] Skipping shouldRespond evaluation; forcing shouldRespond=true`);
        ...
        if (shouldRespond) {
          ...
          if (responseContent && responseContent.actions && responseContent.actions.length > 0) {
            ...
            if (action === 'CREATE_CONNECTION') {
              ...
              logger.debug('[serenapp] IGNORE action: no callback sent');
            ...
            logger.debug('[serenapp] No valid action found, using processActions');
            await runtime.processActions(message, responseMessages, state, callback);
          }
          await runtime.evaluate(message, state, shouldRespond, callback, responseMessages);
        } else {
          // Handle the case where the agent decided not to respond
          logger.debug('[serenapp] Agent decided not to respond (shouldRespond is false).');
          ...
          if (!message.id) {
            logger.error('[serenapp] Message ID is missing, cannot create ignore response.');
            return;
          }
...
const reactionReceivedHandler = async ({
  ...
  try {
    ...
    if (error.code === '23505') {
      logger.warn('[serenapp] Duplicate reaction memory, skipping');
      return;
    }
    logger.error(`[serenapp] Error in reaction handler: ${error}`);
  }
};
...
const messageDeletedHandler = async ({
  ...
  try {
    if (!message.id) {
      logger.error('[serenapp] Cannot delete memory: message ID is missing');
      return;
    }
...
const channelClearedHandler = async ({
  ...
  try {
    logger.info(
      `[serenapp] Clearing ${memoryCount} message memories from channel ${channelId} -> room ${roomId}`
    );
...
const postGeneratedHandler = async ({
  ...
  logger.info('[serenapp] Generating new post...');
  ...
  if (RM) {
    for (const m of RM.data.recentMessages) {
      if (cleanedText === m.content.text) {
        logger.info(`[serenapp] Already recently posted that, retrying: ${cleanedText}`);
  ...
  if (
    ...
    logger.info(`[serenapp] Got prompt moderation refusal, retrying: ${cleanedText}`);
    postGeneratedHandler({
      runtime,
      callback,
      worldId,
...
const syncSingleUser = async (
  ...
  try {
    ...
    logger.info(`[serenapp] Syncing user: ${entity?.metadata?.username || entityId}`);

    // Ensure we're not using WORLD type and that we have a valid channelId
    if (!channelId) {
      logger.warn(`[serenapp] Cannot sync user ${entity?.id} without a valid channelId`);
      return;
    }
    ...
    logger.info(
      `[serenapp] syncSingleUser - type: ${type}, isDM: ${type === ChannelType.DM}, worldMetadata: ${JSON.stringify(worldMetadata)}`
    );
    ...
    try {
      ...
      logger.info(
        `[serenapp] Created world check - ID: ${worldId}, metadata: ${JSON.stringify(createdWorld?.metadata)}`
      );
    } catch (error) {
      logger.error(`[serenapp] Failed to verify created world: ${error}`);
    }

    logger.success(`[serenapp] Successfully synced user: ${entity?.id}`);
  } catch (error) {
    logger.error(
      `[serenapp] Error syncing user: ${error instanceof Error ? error.message : String(error)}`
    );
  }
};
...
const handleServerSync = async ({
  ...
  logger.debug(`[serenapp] Handling server sync event for server: ${world.name}`);
  try {
...
const events = {
  ...
  [EventType.MESSAGE_SENT]: [
    async (payload: MessagePayload) => {
      logger.debug(`[serenapp] Message sent: ${payload.message.content.text}`);
    },
  ],
  ...
  [EventType.ENTITY_JOINED]: [
    async (payload: EntityPayload) => {
      logger.debug(`[serenapp] ENTITY_JOINED event received for entity ${payload.entityId}`);

      if (!payload.worldId) {
        logger.error('[serenapp] No worldId provided for entity joined');
        return;
      }
      if (!payload.roomId) {
        logger.error('[serenapp] No roomId provided for entity joined');
        return;
      }
      if (!payload.metadata?.type) {
        logger.error('[serenapp] No type provided for entity joined');
        return;
      }
  ...
  [EventType.ENTITY_LEFT]: [
    async (payload: EntityPayload) => {
      try {
        ...
        logger.info(`[serenapp] User ${payload.entityId} left world ${payload.worldId}`);
      } catch (error: any) {
        logger.error(`[serenapp] Error handling user left: ${error.message}`);
      }
    },
  ],
  ...
  [EventType.ACTION_STARTED]: [
    async (payload: ActionEventPayload) => {
      logger.debug(`[serenapp] Action started: ${payload.actionName} (${payload.actionId})`);
    },
  ],
  ...
  [EventType.ACTION_COMPLETED]: [
    async (payload: ActionEventPayload) => {
      ...
      logger.debug(`[serenapp] Action ${status}: ${payload.actionName} (${payload.actionId})`);
    },
  ],
  ...
  [EventType.EVALUATOR_STARTED]: [
    async (payload: EvaluatorEventPayload) => {
      logger.debug(
        `[serenapp] Evaluator started: ${payload.evaluatorName} (${payload.evaluatorId})`
      );
    },
  ],
  ...
  [EventType.EVALUATOR_COMPLETED]: [
    async (payload: EvaluatorEventPayload) => {
      const status = payload.error ? `failed: ${payload.error.message}` : 'completed';
      logger.debug(
        `[serenapp] Evaluator ${status}: ${payload.evaluatorName} (${payload.evaluatorId})`
      );
    },
  ],
