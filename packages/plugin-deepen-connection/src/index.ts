...
export async function processAttachments(
  ...
  logger.debug(`[deepen-connection] Processing ${attachments.length} attachment(s)`);
  ...
  for (const attachment of attachments) {
    try {
      ...
      if (attachment.contentType === ContentType.IMAGE && !attachment.description) {
        logger.debug(`[deepen-connection] Generating description for image: ${attachment.url}`);
        ...
        try {
          ...
          if (typeof response === 'string') {
            ...
            if (parsedXml?.description && parsedXml?.text) {
              ...
              logger.debug(
                `[deepen-connection] Generated description: ${processedAttachment.description?.substring(0, 100)}...`
              );
            } else {
              logger.warn(`[deepen-connection] Failed to parse XML response for image description`);
            }
            ...
            logger.warn(`[deepen-connection] Unexpected response format for image description`);
          }
        } catch (error) {
          logger.error(
            `[deepen-connection] Error generating image description: ${error instanceof Error ? error.message : String(error)}`
          );
        ...
        if (isPlainText) {
          logger.debug(`[deepen-connection] Processing plain text document: ${attachment.url}`);
          ...
          logger.debug(
            `[deepen-connection] Extracted text content (first 100 chars): ${processedAttachment.text?.substring(0, 100)}...`
          );
        } else {
          logger.warn(`[deepen-connection] Skipping non-plain-text document: ${contentType}`);
        }
      }
      ...
      logger.error(
        `[deepen-connection] Failed to process attachment ${attachment.url}: ${error instanceof Error ? error.message : String(error)}`
      );
    ...
    const processingPromise = (async () => {
      try {
        if (message.entityId === runtime.agentId) {
          logger.debug(`[deepen-connection] Skipping message from self (${runtime.agentId})`);
          throw new Error('Message is from the agent itself');
        }

        logger.debug(
          `[deepen-connection] Processing message: ${truncateToCompleteSentence(message.content.text || '', 50)}...`
        );

        // First, save the incoming message
        logger.debug('[deepen-connection] Saving message to memory and embeddings');
        await Promise.all([
        ...
        if (
          ...
          logger.debug(`[deepen-connection] Ignoring muted room ${message.roomId}`);
          return;
        }
        ...
        if (!shouldSkipShouldRespond) {
          ...
          logger.debug(
            `[deepen-connection] Evaluating response for ${runtime.character.name}\nPrompt: ${shouldRespondPrompt}`
          );
          ...
          logger.debug(
            `[deepen-connection] Response evaluation for ${runtime.character.name}:\n${response}`
          );
          logger.debug(`[deepen-connection] Response type: ${typeof response}`);
          ...
          logger.debug(`[deepen-connection] Parsed response: ${JSON.stringify(responseObject)}`);
          ...
          logger.debug(
            `[deepen-connection] Skipping shouldRespond check for ${runtime.character.name} because ${room?.type} ${room?.source}`
          );
          shouldRespond = true;
        }
        ...
        if (shouldRespond) {
          ...
          while (retries < maxRetries && (!responseContent?.thought || !responseContent?.actions)) {
            ...
            // logger.debug('[deepen-connection] *** Raw LLM Response ***\n', response);

            // Attempt to parse the XML response
            const parsedXml = parseKeyValueXml(response);
            // logger.debug('[deepen-connection] *** Parsed XML Content ***\n', parsedXml);
            ...
            if (!responseContent?.thought || !responseContent?.actions) {
              logger.warn(
                `[deepen-connection] *** Missing required fields (thought or actions), retrying... ***\nResponse: ${
          ...
          // Simplified action handling for Deepen-Connection Web plugin
          if (responseContent && responseContent.actions && responseContent.actions.length > 0) {
            ...
            if (action === 'NONE' && responseContent.text) {
              ...
              logger.debug(
                `[deepen-connection] Fallback: using processActions for action: ${String(action)}`
              );
              await runtime.processActions(message, responseMessages, state, callback);
            }
          } else {
            // No valid action found, fallback to processActions
            logger.debug('[deepen-connection] No valid action found, using processActions');
            await runtime.processActions(message, responseMessages, state, callback);
          }
          ...
          logger.debug(`[deepen-connection] Saved ignore response to memory: ${ignoreMemory.id}`);
...
const reactionReceivedHandler = async ({
  ...
  try {
    ...
    if (error.code === '23505') {
      logger.warn('[deepen-connection] Duplicate reaction memory, skipping');
      return;
    }
    logger.error('[deepen-connection] Error in reaction handler:', error);
  }
};
...
const messageDeletedHandler = async ({
  ...
  try {
    if (!message.id) {
      logger.error('[deepen-connection] Cannot delete memory: message ID is missing');
      return;
    }
    ...
    logger.error(
      `[deepen-connection] Error in message deleted handler: ${
...
const channelClearedHandler = async ({
  ...
  try {
    logger.info(
      `[deepen-connection] Clearing ${memoryCount} message memories from channel ${channelId} -> room ${roomId}`
    );
    ...
    for (const memory of memories) {
      if (memory.id) {
        try {
          ...
          logger.warn(
            `[deepen-connection] Failed to delete message memory ${memory.id}: ${
              error instanceof Error ? error.message : String(error)
            }`
          );
        }
    ...
    logger.info(
      `[deepen-connection] Successfully cleared ${deletedCount}/${memories.length} message memories from channel ${channelId}`
    );
  } catch (error: unknown) {
    logger.error(
      `[deepen-connection] Error in channel cleared handler: ${
        error instanceof Error ? error.message : String(error)
      }`
    );
  }
...
const postGeneratedHandler = async ({
  ...
  logger.info('[deepen-connection] Generating new post...');
  ...
  while (retries < maxRetries && (!responseContent?.thought || !responseContent?.actions)) {
    ...
    if (!responseContent?.thought || !responseContent?.actions) {
      logger.warn(
        `[deepen-connection] *** Missing required fields, retrying... ***\nResponse: ${
          typeof response === 'string' ? response : JSON.stringify(response)
        }\nParsedXml: ${JSON.stringify(parsedXml)}\nResponseContent: ${JSON.stringify(responseContent)}`
      );
    }
  ...
  if (!parsedXmlResponse) {
    logger.error(
      `[deepen-connection] Failed to parse XML response for post creation. Raw response: ${
  ...
  if (RM) {
    for (const m of RM.data.recentMessages) {
      if (cleanedText === m.content.text) {
        logger.log(`[deepen-connection] Already recently posted that, retrying: ${cleanedText}`);
  ...
  if (
    ...
    logger.log(`[deepen-connection] Got prompt moderation refusal, retrying: ${cleanedText}`);
    postGeneratedHandler({
      runtime,
      callback,
      worldId,
...
const syncSingleUser = async (
  ...
  try {
    ...
    logger.info(`[deepen-connection] Syncing user: ${entity?.metadata?.username || entityId}`);

    // Ensure we're not using WORLD type and that we have a valid channelId
    if (!channelId) {
      logger.warn(`[deepen-connection] Cannot sync user ${entity?.id} without a valid channelId`);
      return;
    }
    ...
    logger.info(
      `[deepen-connection] syncSingleUser - type: ${type}, isDM: ${type === ChannelType.DM}, worldMetadata: ${JSON.stringify(worldMetadata)}`
    );
    ...
    try {
      ...
      logger.info(
        `[deepen-connection] Created world check - ID: ${worldId}, metadata: ${JSON.stringify(createdWorld?.metadata)}`
      );
    } catch (error) {
      logger.error(`[deepen-connection] Failed to verify created world: ${error}`);
    }

    logger.success(`[deepen-connection] Successfully synced user: ${entity?.id}`);
  } catch (error) {
    logger.error(
      `[deepen-connection] Error syncing user: ${error instanceof Error ? error.message : String(error)}`
    );
  }
};
...
const handleServerSync = async ({
  ...
  logger.debug(`[deepen-connection] Handling server sync event for server: ${world.name}`);
  try {
...
const events = {
  ...
  [EventType.MESSAGE_SENT]: [
    async (payload: MessagePayload) => {
      logger.debug(`[deepen-connection] Message sent: ${payload.message.content.text}`);
    },
  ],
  ...
  [EventType.ENTITY_JOINED]: [
    async (payload: EntityPayload) => {
      ...
      if (!payload.roomId) {
        logger.error('[deepen-connection] No roomId provided for entity joined');
        return;
      }
      if (!payload.metadata?.type) {
        logger.error('[deepen-connection] No type provided for entity joined');
        return;
      }
  ...
  [EventType.ENTITY_LEFT]: [
    async (payload: EntityPayload) => {
      try {
        ...
        logger.info(`[deepen-connection] User ${payload.entityId} left world ${payload.worldId}`);
      } catch (error: any) {
        logger.error(`[deepen-connection] Error handling user left: ${error.message}`);
      }
    },
  ],
  ...
  [EventType.EVALUATOR_STARTED]: [
    async (payload: EvaluatorEventPayload) => {
      logger.debug(
        `[deepen-connection] Evaluator started: ${payload.evaluatorName} (${payload.evaluatorId})`
      );
    },
  ],
  ...
  [EventType.EVALUATOR_COMPLETED]: [
    async (payload: EvaluatorEventPayload) => {
      const status = payload.error ? `failed: ${payload.error.message}` : 'completed';
      logger.debug(
        `[deepen-connection] Evaluator ${status}: ${payload.evaluatorName} (${payload.evaluatorId})`
      );
    },
  ],
