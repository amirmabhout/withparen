...
export class TaskService extends Service {
  ...
  private readonly TICK_INTERVAL = 1000 * 60 * 14; // Check every 15 minutes
  ...
  async createTestTasks() {
    ...
    this.runtime.registerTaskWorker({
      ...
      execute: async (runtime, _options) => {
        try {
          logger.info('[Deepen-Connection] Executing daily check-in task');
          ...
          logger.info(`[Deepen-Connection] Found ${allRoomIds.length} rooms for daily check-in`);
    ...
    try {
      ...
      logger.warn('[Deepen-Connection] Using default world for task creation');
      worldId = '00000000-0000-0000-0000-000000000000' as UUID;
    }
    ...
    if (dailyCheckinTasks.length === 0) {
      ...
      logger.info('[Deepen-Connection] Created daily check-in task');
    }
