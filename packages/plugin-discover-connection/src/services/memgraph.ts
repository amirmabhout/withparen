      ...
      (await this.withSession(async (session) => {
        try {
          ...
          const query = `
            ...
            MATCH (p)-[:MANAGED_BY]->(agent:Agent {agentId: $agentId})
            WHERE p.userStatus = 'active'
              AND NOT p.entityid IN $excludeIds
              ${activeUserIds.size > 0 ? 'AND p.entityid IN $activeUserIds' : ''}
            OPTIONAL MATCH (p)-[activeMatch:MATCHED_WITH]-()
            WHERE activeMatch IS NULL OR NOT activeMatch.status IN ['match_found', 'proposal_sent', 'accepted', 'scheduled']
            RETURN p.entityid as entityId,
