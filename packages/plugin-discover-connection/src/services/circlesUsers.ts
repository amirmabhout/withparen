...
export class CirclesUsersService extends Service {
  ...
  async getTrustCounts(
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (isLastAttempt) {
          logger.error(
            `[discover-connection] Error getting trust counts for ${userAddress} after ${maxRetries + 1} attempts: ${error}`
          );
        ...
        logger.warn(
          `[discover-connection] Retry ${attempt + 1}/${maxRetries} for ${userAddress} after ${Math.round(delay)}ms delay. Error: ${error}`
        );
        await sleep(delay);
      }
    }
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if ((i + 1) % 25 === 0 || i === rows.length - 1) {
          ...
          logger.info(
            `[discover-connection] Processed ${i + 1}/${rows.length} users (${withTrustCount} with trusts, ${zeroTrustCount} zero-trust, ${failedCount} failed)`
          );
        }
      ...
      if (failedUsers.length > 0) {
        logger.warn(
          `[discover-connection] ${failedUsers.length} users failed trust verification lookup in this batch`
        );
      }
      if (zeroTrustUsers.length > 0) {
        logger.info(
          `[discover-connection] ${zeroTrustUsers.length} users have zero trust connections (new/inactive users)`
        );
      }
      ...
      const zeroTrustCount = usersWithTrustData.filter(
        (u) => !u.isVerified && u.trustCount === 0
      ).length;

      logger.info(
        `[discover-connection] Batch summary: ${verifiedCount} verified, ${unverifiedCount - zeroTrustCount} unverified (with some trusts), ${zeroTrustCount} zero-trust users, ${failedUsers.length} failures`
      );
      logger.info(
    ...
    try {
          ...
          if (users.length === 0) {
            ...
            const trulyNewUsers = users.filter((user) => {
              const existingUser = existingUsers.find(
                (existing) => existing.avatar === user.avatar
              );
              if (!existingUser) {
      ...
      if (cursor) {
        ...
        const newBoundaries: CirclesCursorBoundaries = {
          highCursor: {
            // Update high cursor to the latest processed position
            blockNumber: cursor.blockNumber,
            transactionIndex: cursor.transactionIndex,
            logIndex: cursor.logIndex,
          },
          lowCursor: existingBoundaries?.lowCursor || {
            // Preserve existing low cursor
            blockNumber: cursor.blockNumber,
            transactionIndex: cursor.transactionIndex,
            logIndex: cursor.logIndex,
          },
  ...
  async clearAllCache(): Promise<void> {
    try {
      ...
      logger.info(
        '[discover-connection] Cleared all Circles users cache - next update will be a complete refresh with proper cursor tracking'
      );
    } catch (error) {
