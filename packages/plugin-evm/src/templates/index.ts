...
export const trustTemplate = `Given the recent messages and wallet information below:

{{recentMessages}}

Extract the following information about the requested trust operation:
- Wallet address to trust (must be a valid Ethereum address)
- Chain to execute on (must be one of the supported chains, typically gnosis for Circles)
- Expiry timestamp (optional, defaults to permanent trust)

Respond with an XML block containing only the extracted values. Use null for any values that cannot be determined.

<response>
    <trustReceiver>string | null</trustReceiver>
    <chain>{{supportedChains}} | null</chain>
    <expiry>string | null</expiry>
</response>
