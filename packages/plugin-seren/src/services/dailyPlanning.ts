...
export class DailyPlanningService extends Service {
    ...
    this.runtime.registerTaskWorker({
      ...
      validate: async (_runtime, _message, _state) => {
        ...
        if (isPlanningTime && hasNotPlannedToday) {
          logger.debug(
            '[Seren] Daily planning validation: It is planning time and planning not done today, task should run'
          );
          return true;
        }
  ...
  async executeDailyPlanning(runtime: IAgentRuntime) {
    try {
      ...
      logger.info(
        `[Seren] Found ${activeConnections.length} active connections for daily planning`
      );
      ...
      for (const connectionData of activeConnections) {
        try {
          ...
          logger.debug(
            `[Seren] Queued planning task for connection: ${connectionData.connection.partners.join(' & ')}`
          );
        } catch (error) {
  ...
  async executeSingleConnectionPlanning(runtime: IAgentRuntime, connectionData: any) {
    try {
      ...
      if (planningResult) {
        ...
        logger.info(
          `[Seren] Successfully generated daily plans and scheduled check-ins for ${person1.name} and ${person2.name}`
        );
      } else {
    ...
    try {
      ...
      const personaMemoryPromises = personaDimensions.map(async (tableName) => {
        try {
          ...
          logger.debug(
            `[Seren] Found ${memories.length} memories in ${tableName} for user ${userId}`
          );
          return memories;
        } catch (error) {
      ...
      logger.debug(
        `[Seren] Total persona memories found for user ${userId}: ${allPersonaMemories.length}`
      );
  ...
  private async getConnectionMemories(runtime: IAgentRuntime, userId: UUID): Promise<string> {
    try {
      ...
      logger.debug(
        `[Seren] Total connection memories found for user ${userId}: ${allConnectionMemories.length}`
      );
    ...
    try {
      ...
      if (room && room.source) {
        try {
          ...
          logger.debug(
            `[Seren] Personalized check-in sent successfully to user: ${userId} (${room.source})`
          );
        } catch (sendError) {
          // If direct sending fails, try emitting an event
          logger.debug(
            `[Seren] Direct send failed for user ${userId}, trying event emission:`,
            sendError
          );
