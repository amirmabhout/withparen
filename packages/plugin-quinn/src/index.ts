...
export async function processAttachments(
  ...
  logger.debug(`[quinn] Processing ${attachments.length} attachment(s)`);
  ...
  for (const attachment of attachments) {
    try {
      ...
      if (attachment.contentType === ContentType.IMAGE && !attachment.description) {
        logger.debug(`[quinn] Generating description for image: ${attachment.url}`);
        ...
        try {
          ...
          if (typeof response === 'string') {
            ...
            if (parsedXml?.description && parsedXml?.text) {
              ...
              logger.debug(
                `[quinn] Generated description: ${processedAttachment.description?.substring(0, 100)}...`
              );
            } else {
              logger.warn(`[quinn] Failed to parse XML response for image description`);
            }
            ...
            logger.warn(`[quinn] Unexpected response format for image description`);
          }
        } catch (error) {
          logger.error(`[quinn] Error generating image description: ${error}`);
        ...
        if (isPlainText) {
          logger.debug(`[quinn] Processing plain text document: ${attachment.url}`);
          ...
          logger.debug(
            `[quinn] Extracted text content (first 100 chars): ${processedAttachment.text?.substring(0, 100)}...`
          );
        } else {
          logger.warn(`[quinn] Skipping non-plain-text document: ${contentType}`);
        }
      }

      processedAttachments.push(processedAttachment);
    } catch (error) {
      logger.error(`[quinn] Failed to process attachment ${attachment.url}: ${error}`);
      // Add the original attachment if processing fails
      processedAttachments.push(attachment);
    }
  }

  logger.debug(`[quinn] Processed attachments: ${JSON.stringify(processedAttachments)}`);
...
const messageReceivedHandler = async ({
  ...
  try {
    logger.info(`[quinn] Message received from ${message.entityId} in room ${message.roomId}`);
    ...
    const processingPromise = (async () => {
      try {
        if (message.entityId === runtime.agentId) {
          logger.debug(`[quinn] Skipping message from self (${runtime.agentId})`);
          throw new Error('Message is from the agent itself');
        }

        logger.debug(
          `[quinn] Processing message: ${truncateToCompleteSentence(message.content.text || '', 50)}...`
        );

        // First, save the incoming message
        logger.debug('[quinn] Saving message to memory and embeddings');
        await Promise.all([
          runtime.addEmbeddingToMemory(message),
          runtime.createMemory(message, 'messages'),
        ]);

        // Quinn focuses on connection discovery, no special authentication handling needed
        ...
        if (
          ...
          logger.debug(`[quinn] Ignoring muted room ${message.roomId}`);
          return;
        }
        ...
        logger.debug(`[quinn] Skipping shouldRespond evaluation; forcing shouldRespond=true`);
        ...
        if (shouldRespond) {
          ...
          while (retries < maxRetries && (!responseContent?.thought || !responseContent?.actions)) {
            ...
            logger.debug(`[quinn] *** Raw LLM Response ***\n${response}`);

            // Attempt to parse the XML response
            const parsedXml = parseKeyValueXml(response);
            logger.debug(`[quinn] *** Parsed XML Content ***\n${JSON.stringify(parsedXml)}`);
          ...
          if (responseContent && responseContent.actions && responseContent.actions.length > 0) {
            ...
            if (action === 'CREATE_CONNECTION') {
              ...
              logger.debug('[quinn] NONE action: sending callback with message');
              await callback(responseContent);
            } else if (action === 'IGNORE') {
              // IGNORE action: no callback, no action
              logger.debug('[quinn] IGNORE action: no callback sent');
              // Do nothing - no callback, no message sent
            } else {
              // Fallback to original logic for any other actions (future-proofing)
              logger.debug(`[quinn] Fallback: using processActions for action: ${action}`);
              await runtime.processActions(message, responseMessages, state, callback);
            }
          } else {
            // No valid action found, fallback to processActions
            logger.debug('[quinn] No valid action found, using processActions');
            await runtime.processActions(message, responseMessages, state, callback);
          }
          await runtime.evaluate(message, state, shouldRespond, callback, responseMessages);
        } else {
          // Handle the case where the agent decided not to respond
          logger.debug('[quinn] Agent decided not to respond (shouldRespond is false).');
          ...
          if (!message.id) {
            logger.error('[quinn] Message ID is missing, cannot create ignore response.');
            return;
          }
...
const reactionReceivedHandler = async ({
  ...
  try {
    ...
    if (error.code === '23505') {
      logger.warn('[quinn] Duplicate reaction memory, skipping');
      return;
    }
    logger.error(`[quinn] Error in reaction handler: ${error}`);
  }
};
...
const messageDeletedHandler = async ({
  ...
  try {
    if (!message.id) {
      logger.error('[quinn] Cannot delete memory: message ID is missing');
      return;
    }

    logger.info(`[quinn] Deleting memory for message ${message.id} from room ${message.roomId}`);
    await runtime.deleteMemory(message.id);
    logger.debug(`[quinn] Successfully deleted memory for message ${message.id}`);
  } catch (error: unknown) {
    logger.error(`[quinn] Error in message deleted handler: ${error}`);
  }
};
...
const channelClearedHandler = async ({
  ...
  try {
    logger.info(
      `[quinn] Clearing ${memoryCount} message memories from channel ${channelId} -> room ${roomId}`
    );
    ...
    for (const memory of memories) {
      if (memory.id) {
        try {
          ...
          logger.warn(`[quinn] Failed to delete message memory ${memory.id}: ${error}`);
        }
      }
    }

    logger.info(
      `[quinn] Successfully cleared ${deletedCount}/${memories.length} message memories from channel ${channelId}`
    );
  } catch (error: unknown) {
    logger.error(`[quinn] Error in channel cleared handler: ${error}`);
  }
};
...
const postGeneratedHandler = async ({
  ...
  logger.info('[quinn] Generating new post...');
  ...
  if (RM) {
    for (const m of RM.data.recentMessages) {
      if (cleanedText === m.content.text) {
        logger.info(`[quinn] Already recently posted that, retrying: ${cleanedText}`);
  ...
  if (
    ...
    logger.info(`[quinn] Got prompt moderation refusal, retrying: ${cleanedText}`);
    postGeneratedHandler({
      runtime,
      callback,
      worldId,
...
const syncSingleUser = async (
  ...
  try {
    ...
    logger.info(`[quinn] Syncing user: ${entity?.metadata?.username || entityId}`);

    // Ensure we're not using WORLD type and that we have a valid channelId
    if (!channelId) {
      logger.warn(`[quinn] Cannot sync user ${entity?.id} without a valid channelId`);
      return;
    }
    ...
    logger.info(
      `[quinn] syncSingleUser - type: ${type}, isDM: ${type === ChannelType.DM}, worldMetadata: ${JSON.stringify(worldMetadata)}`
    );
    ...
    try {
      ...
      logger.info(
        `[quinn] Created world check - ID: ${worldId}, metadata: ${JSON.stringify(createdWorld?.metadata)}`
      );
    } catch (error) {
      logger.error(`[quinn] Failed to verify created world: ${error}`);
    }

    logger.success(`[quinn] Successfully synced user: ${entity?.id}`);
  } catch (error) {
    logger.error(
      `[quinn] Error syncing user: ${error instanceof Error ? error.message : String(error)}`
    );
  }
};
...
const handleServerSync = async ({
  ...
  logger.debug(`[quinn] Handling server sync event for server: ${world.name}`);
  try {
...
const events = {
  ...
  [EventType.MESSAGE_SENT]: [
    async (payload: MessagePayload) => {
      logger.debug(`[quinn] Message sent: ${payload.message.content.text}`);
    },
  ],
  ...
  [EventType.ENTITY_JOINED]: [
    async (payload: EntityPayload) => {
      logger.debug(`[quinn] ENTITY_JOINED event received for entity ${payload.entityId}`);

      if (!payload.worldId) {
        logger.error('[quinn] No worldId provided for entity joined');
        return;
      }
      if (!payload.roomId) {
        logger.error('[quinn] No roomId provided for entity joined');
        return;
      }
      if (!payload.metadata?.type) {
        logger.error('[quinn] No type provided for entity joined');
        return;
      }
  ...
  [EventType.ENTITY_LEFT]: [
    async (payload: EntityPayload) => {
      try {
        ...
        logger.info(`[quinn] User ${payload.entityId} left world ${payload.worldId}`);
      } catch (error: any) {
        logger.error(`[quinn] Error handling user left: ${error.message}`);
      }
    },
  ],
  ...
  [EventType.ACTION_STARTED]: [
    async (payload: ActionEventPayload) => {
      logger.debug(`[quinn] Action started: ${payload.actionName} (${payload.actionId})`);
    },
  ],
  ...
  [EventType.ACTION_COMPLETED]: [
    async (payload: ActionEventPayload) => {
      ...
      logger.debug(`[quinn] Action ${status}: ${payload.actionName} (${payload.actionId})`);
    },
  ],
  ...
  [EventType.EVALUATOR_COMPLETED]: [
    async (payload: EvaluatorEventPayload) => {
      const status = payload.error ? `failed: ${payload.error.message}` : 'completed';
      logger.debug(
        `[quinn] Evaluator ${status}: ${payload.evaluatorName} (${payload.evaluatorId})`
      );
    },
  ],
