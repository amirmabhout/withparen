  ...
  try {
    ...
    if (skipIfExists) {
      try {
        ...
        if (testDataMemories.length >= userCount) {
          logger.info(
            `[quinn] ${testDataMemories.length} test data records already exist in room ${roomId}, skipping seeding`
          );
          return testDataMemories.slice(0, userCount);
        } else if (testDataMemories.length > 0) {
          logger.info(
            `[quinn] Found ${testDataMemories.length} existing test records, but need ${userCount}. Creating additional records...`
          );
    ...
    for (const user of usersToCreate) {
      ...
      try {
        ...
        logger.debug(
          `[quinn] ✅ Generated embedding with length: ${memoryWithEmbedding.embedding.length}`
        );
        ...
        logger.debug(
          `[quinn]   - Metadata name: ${(createdMemory.content?.metadata as any)?.name}`
        );
    ...
    logger.info(
      `[quinn] 🎉 Test data seeding completed! Created ${createdMemories.length} sample users`
    );
 ...
 */
export async function cleanupQuinnTestData(runtime: IAgentRuntime, roomId?: string): Promise<void> {
  ...
  try {
    ...
    logger.warn(
      `[quinn] Test data cleanup not fully implemented. Consider manual cleanup if needed.`
    );
    logger.info(`[quinn] To manually clean test data:`);
    logger.info(
      `[quinn]   1. Check persona_contexts table for entries with metadata.isTestData = true`
    );
