...
export async function processAttachments(
  ...
  logger.debug(`[serenweb] Processing ${attachments.length} attachment(s)`);
  ...
  for (const attachment of attachments) {
    try {
      ...
      if (attachment.contentType === ContentType.IMAGE && !attachment.description) {
        logger.debug(`[serenweb] Generating description for image: ${attachment.url}`);
        ...
        try {
          ...
          if (typeof response === 'string') {
            ...
            if (parsedXml?.description && parsedXml?.text) {
              ...
              logger.debug(
                `[serenweb] Generated description: ${processedAttachment.description?.substring(0, 100)}...`
              );
            } else {
              logger.warn(`[serenweb] Failed to parse XML response for image description`);
            }
            ...
            logger.warn(`[serenweb] Unexpected response format for image description`);
          }
        } catch (error) {
          logger.error(`[serenweb] Error generating image description:`, error);
        ...
        if (isPlainText) {
          logger.debug(`[serenweb] Processing plain text document: ${attachment.url}`);
          ...
          logger.debug(
            `[serenweb] Extracted text content (first 100 chars): ${processedAttachment.text?.substring(0, 100)}...`
          );
        } else {
          logger.warn(`[serenweb] Skipping non-plain-text document: ${contentType}`);
        }
      }

      processedAttachments.push(processedAttachment);
    } catch (error) {
      logger.error(`[serenweb] Failed to process attachment ${attachment.url}:`, error);
...
const messageReceivedHandler = async ({
  ...
  try {
    logger.info(`[serenweb] Message received from ${message.entityId} in room ${message.roomId}`);
    ...
    const processingPromise = (async () => {
      try {
        if (message.entityId === runtime.agentId) {
          logger.debug(`[serenweb] Skipping message from self (${runtime.agentId})`);
          throw new Error('Message is from the agent itself');
        }

        logger.debug(
          `[serenweb] Processing message: ${truncateToCompleteSentence(message.content.text || '', 50)}...`
        );

        // First, save the incoming message
        logger.debug('[serenweb] Saving message to memory and embeddings');
        await Promise.all([
        ...
        if (
          ...
          logger.debug(`[serenweb] Ignoring muted room ${message.roomId}`);
          return;
        }
        ...
        if (!shouldSkipShouldRespond) {
          ...
          logger.debug(
            `[serenweb] Evaluating response for ${runtime.character.name}\nPrompt: ${shouldRespondPrompt}`
          );
          ...
          logger.debug(
            `[serenweb] Response evaluation for ${runtime.character.name}:\n${response}`
          );
          logger.debug(`[serenweb] Response type: ${typeof response}`);
          ...
          logger.debug('[serenweb] Parsed response:', responseObject);
          ...
          logger.debug(
            `[serenweb] Skipping shouldRespond check for ${runtime.character.name} because ${room?.type} ${room?.source}`
          );
          shouldRespond = true;
        }
        ...
        if (shouldRespond) {
          ...
          while (retries < maxRetries && (!responseContent?.thought || !responseContent?.actions)) {
            ...
            logger.debug('[serenweb] *** Raw LLM Response ***\n', response);

            // Attempt to parse the XML response
            const parsedXml = parseKeyValueXml(response);
            logger.debug('[serenweb] *** Parsed XML Content ***\n', parsedXml);
            ...
            if (!responseContent?.thought || !responseContent?.actions) {
              logger.warn(
                '[serenweb] *** Missing required fields (thought or actions), retrying... ***\n',
                response,
                parsedXml,
                responseContent
              );
          ...
          if (responseContent && responseContent.actions && responseContent.actions.length > 0) {
            ...
            if (action === 'NONE' && responseContent.text) {
              ...
              logger.debug('[serenweb] NONE action: sending callback with message');
              await callback(responseContent);
            } else if (action === 'IGNORE') {
              // IGNORE action: no callback, no action
              logger.debug('[serenweb] IGNORE action: no callback sent');
              // Do nothing - no callback, no message sent
            } else {
              // Fallback to original logic for any other actions (future-proofing)
              logger.debug('[serenweb] Fallback: using processActions for action:', action);
              await runtime.processActions(message, responseMessages, state, callback);
            }
          } else {
            // No valid action found, fallback to processActions
            logger.debug('[serenweb] No valid action found, using processActions');
            await runtime.processActions(message, responseMessages, state, callback);
          }
          await runtime.evaluate(message, state, shouldRespond, callback, responseMessages);
        } else {
          // Handle the case where the agent decided not to respond
          logger.debug('[serenweb] Agent decided not to respond (shouldRespond is false).');
          ...
          if (!message.id) {
            logger.error('[serenweb] Message ID is missing, cannot create ignore response.');
            return;
          }
          ...
          logger.debug('[serenweb] Saved ignore response to memory', {
...
const reactionReceivedHandler = async ({
  ...
  try {
    ...
    if (error.code === '23505') {
      logger.warn('[serenweb] Duplicate reaction memory, skipping');
      return;
    }
    logger.error('[serenweb] Error in reaction handler:', error);
  }
};
...
const messageDeletedHandler = async ({
  ...
  try {
    if (!message.id) {
      logger.error('[serenweb] Cannot delete memory: message ID is missing');
      return;
    }

    logger.info('[serenweb] Deleting memory for message', message.id, 'from room', message.roomId);
    await runtime.deleteMemory(message.id);
    logger.debug('[serenweb] Successfully deleted memory for message', message.id);
  } catch (error: unknown) {
    logger.error('[serenweb] Error in message deleted handler:', error);
  }
};
...
const channelClearedHandler = async ({
  ...
  try {
    logger.info(
      `[serenweb] Clearing ${memoryCount} message memories from channel ${channelId} -> room ${roomId}`
    );
    ...
    for (const memory of memories) {
      if (memory.id) {
        try {
          ...
          logger.warn(`[serenweb] Failed to delete message memory ${memory.id}:`, error);
        }
      }
    }

    logger.info(
      `[serenweb] Successfully cleared ${deletedCount}/${memories.length} message memories from channel ${channelId}`
    );
  } catch (error: unknown) {
    logger.error('[serenweb] Error in channel cleared handler:', error);
  }
};
...
const postGeneratedHandler = async ({
  ...
  logger.info('[serenweb] Generating new post...');
  ...
  while (retries < maxRetries && (!responseContent?.thought || !responseContent?.actions)) {
    ...
    if (!responseContent?.thought || !responseContent?.actions) {
      logger.warn(
        '[serenweb] *** Missing required fields, retrying... ***\n',
        response,
        parsedXml,
        responseContent
      );
  ...
  if (!parsedXmlResponse) {
    logger.error(
      '[serenweb] Failed to parse XML response for post creation. Raw response:',
      xmlResponseText
    );
  ...
  if (RM) {
    for (const m of RM.data.recentMessages) {
      if (cleanedText === m.content.text) {
        logger.log('[serenweb] Already recently posted that, retrying', cleanedText);
  ...
  if (
    ...
    logger.log('[serenweb] Got prompt moderation refusal, retrying', cleanedText);
    postGeneratedHandler({
      runtime,
      callback,
      worldId,
...
const syncSingleUser = async (
  ...
  try {
    ...
    logger.info(`[serenweb] Syncing user: ${entity?.metadata?.username || entityId}`);

    // Ensure we're not using WORLD type and that we have a valid channelId
    if (!channelId) {
      logger.warn(`[serenweb] Cannot sync user ${entity?.id} without a valid channelId`);
      return;
    }
    ...
    logger.info(
      `[serenweb] syncSingleUser - type: ${type}, isDM: ${type === ChannelType.DM}, worldMetadata: ${JSON.stringify(worldMetadata)}`
    );
    ...
    try {
      ...
      logger.info(
        `[serenweb] Created world check - ID: ${worldId}, metadata: ${JSON.stringify(createdWorld?.metadata)}`
      );
    } catch (error) {
      logger.error(`[serenweb] Failed to verify created world: ${error}`);
    }

    logger.success(`[serenweb] Successfully synced user: ${entity?.id}`);
  } catch (error) {
    logger.error(
      `[serenweb] Error syncing user: ${error instanceof Error ? error.message : String(error)}`
    );
  }
};
...
const handleServerSync = async ({
  ...
  logger.debug(`[serenweb] Handling server sync event for server: ${world.name}`);
  try {
...
const events = {
  ...
  [EventType.MESSAGE_SENT]: [
    async (payload: MessagePayload) => {
      logger.debug(`[serenweb] Message sent: ${payload.message.content.text}`);
    },
  ],
  ...
  [EventType.ENTITY_JOINED]: [
    async (payload: EntityPayload) => {
      logger.debug(`[serenweb] ENTITY_JOINED event received for entity ${payload.entityId}`);

      if (!payload.worldId) {
        logger.error('[serenweb] No worldId provided for entity joined');
        return;
      }
      if (!payload.roomId) {
        logger.error('[serenweb] No roomId provided for entity joined');
        return;
      }
      if (!payload.metadata?.type) {
        logger.error('[serenweb] No type provided for entity joined');
        return;
      }
  ...
  [EventType.ENTITY_LEFT]: [
    async (payload: EntityPayload) => {
      try {
        ...
        logger.info(`[serenweb] User ${payload.entityId} left world ${payload.worldId}`);
      } catch (error: any) {
        logger.error(`[serenweb] Error handling user left: ${error.message}`);
      }
    },
  ],
  ...
  [EventType.ACTION_STARTED]: [
    async (payload: ActionEventPayload) => {
      logger.debug(`[serenweb] Action started: ${payload.actionName} (${payload.actionId})`);
    },
  ],
  ...
  [EventType.ACTION_COMPLETED]: [
    async (payload: ActionEventPayload) => {
      ...
      logger.debug(`[serenweb] Action ${status}: ${payload.actionName} (${payload.actionId})`);
    },
  ],
  ...
  [EventType.EVALUATOR_STARTED]: [
    async (payload: EvaluatorEventPayload) => {
      logger.debug(
        `[serenweb] Evaluator started: ${payload.evaluatorName} (${payload.evaluatorId})`
      );
    },
  ],
  ...
  [EventType.EVALUATOR_COMPLETED]: [
    async (payload: EvaluatorEventPayload) => {
      const status = payload.error ? `failed: ${payload.error.message}` : 'completed';
      logger.debug(
        `[serenweb] Evaluator ${status}: ${payload.evaluatorName} (${payload.evaluatorId})`
      );
    },
  ],
...
export const serenwebPlugin: Plugin = {
  name: 'serenweb',
  description: 'Agent serenweb with basic actions and evaluators',
  actions: [
